...
# Output:
# (a) file that contains leaf area for each leaf and stem of phytomer.
#
...
if len(sys.argv) <= 1:
    print("extract_aream2.py input_from_caribu.ssv")
    sys.exit()
...
try:
    ...
    print("\t$ python extract_aream2.py input_from_caribu.ssv")
    sys.exit()
...
for i in range(len(LILI_caribu_input)):
    ...
    for j in range(len(LILI_caribu_input[i])):
        li_tsv_temp.append(float(LILI_caribu_input[i][0])) # Eabsm2
...
print '\n'.join(('\t'.join(str(i) for i in item[0:])) for item in li_area)
