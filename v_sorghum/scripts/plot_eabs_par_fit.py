...
##### Plot phytomer depth and PAR absorbed by each phytomer
#plt.subplot(3, 1, 1)
#plot(li_1Y, li_1X, '--o', color=small_color, label='vsorghum - small angles', alpha=0.9)
#plot(li_2Y, li_2X, '--o', color=large_color, label='vsorghum - large angles', alpha=0.9)
#plt.xlabel('PAR absorbed by phytomer ' r'($KJ m^{-2} s^{-1}$)')
#plt.ylabel('depth (phytomer)')
...
for phytomer in range(len(li_1Y)):
	...
	li_1Y_absorbed.append(sum(li_1Y[phytomer]))
...
for phytomer in range(len(li_2Y)):
	...
	li_2Y_absorbed.append(sum(li_2Y[phytomer]))

# convert kJ to umol
li_1Y_available = KJ_to_umol(np.array(li_1Y_available))
li_2Y_available = KJ_to_umol(np.array(li_2Y_available))

li_1Y_absorbed = KJ_to_umol(np.array(li_1Y_absorbed))
li_2Y_absorbed = KJ_to_umol(np.array(li_2Y_absorbed))
...
##### Plot phytomer depth and PAR absorbed by each phytomer
plt.subplot(3, 1, 1)
plot(li_1Y_absorbed, li_1X, '--o', color=small_color, label='vsorghum - small angles', alpha=0.9)
plot(li_2Y_absorbed, li_2X, '--o', color=large_color, label='vsorghum - large angles', alpha=0.9)
plt.xlabel('PAR absorbed by phytomer ' r'($\mu$mol $m^{-2} s^{-1}$)')
plt.ylabel('depth (phytomer)')
print(li_1Y_absorbed)
print(li_2Y_absorbed)

##### Plot depth and PAR available
plt.subplot(3, 1, 2)
plot(li_1Y_available, li_1X, '--o', color=small_color, label='vsorghum - small angles', alpha=0.9)
plot(li_2Y_available, li_2X, '--o', color=large_color, label='vsorghum - large angles', alpha=0.9)
...
plt.subplot(3, 1, 3)
plot(np.array(li_1Y_available), li_1X_percent, '--o', color=small_color)
plot(np.array(li_2Y_available), li_2X_percent, '--o', color=large_color)
...
pop1,popc = curve_fit(light_func, (np.array(li_1X_percent), np.array(max(li_1X_percent))) , (li_1Y_available))
k_1, partop_1 = pop1
pop2,popc = curve_fit(light_func, (np.array(li_2X_percent), np.array(max(li_2X_percent))) , (li_2Y_available))
k_2, partop_2 = pop2

##### plot the curve_fit
plot(light_func((np.linspace(0.0, max(li_1X_percent), 200), np.array(max(li_1X_percent))), k_1, partop_1), np.linspace(0., max(li_1X_percent), 200), color=small_color,label='k = %f' %(k_1))
plot(light_func((np.linspace(0.0, max(li_2X_percent), 200), np.array(max(li_2X_percent))), k_2, partop_2), np.linspace(0., max(li_2X_percent), 200), color=large_color,label='k = %f' %(k_2))
